...
```{r}
d_tmm_filtered <- d_tmm_selected_diagnosis %>% filter(cell_type== 'Astrocytes', tissue== 'Deep white matter') %>%
  mutate(group= .[['diagnosis']]) %>% 
  mutate(outcome= .[['Exon count']])
...
# Group <- factor(paste(factor$age, factor$sex, factor$pmi, factor$group, sep= "."))

des <- model.matrix(~sex+age+pmi+group, data= factor)
y_temp <- estimateDisp(y, design= des)
qlfit <- glmQLFit(y_temp, design= des)

# AD vs. Control 
qlf1 <- glmQLFTest(qlfit, coef= 5)
# AD vs. Dementia
qlf2 <- glmQLFTest(qlfit, coef= 6)
# Ctrl vs. Dementia 
qlf3 <- glmQLFTest(qlfit, contrast= c(0, 0, 0, 0, -1, 1))

# comparisons <- list("Ctrl-AD"= c(5), "Dementia-AD"= c(6), "Dementia-Ctrl"= c(0, 0, 0, 0, -1, 1))
# 
# 
# coefs <- matrix(nrow=3) 
# coefs[1, ] <- 5
# coefs[2, ] <- 6
# coefs[3, ] <-  c(0, 0, 0, 0, -1, 1)
...
```{r}
comparisons <- c('groupAD-groupCtrl', 'groupAD-groupDementia', 'groupCtrl-groupDementia')
...
```{r}
comparisons <- c("group0_1-group2_4", "group0_1-group5_6", "group2_4-group5_6")
