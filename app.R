...
for (i in 1:length(reads_dwm_list)) {
  ...
  corr2 <- read.csv(paste0("dwm_fig2", g, ".csv")) %>%
    as.data.frame() %>% merge(data2, by= "cellType", all= TRUE)
  corr3 <- read.csv(paste0("dwm_fig3", g, ".csv")) %>%
    as.data.frame() %>% merge(data3, by= "cellType", all= TRUE)
  corr4 <- read.csv(paste0("dwm_fig4", g, ".csv")) %>%
    as.data.frame() %>% merge(data3, by= "cellType", all= TRUE)
  ...
  p_dwm <- ggplot(data, aes(x=factor(Diagnosis, levels= c("Control", "Case")), y=Proportion)) +
  ...
  p_dwm_2 <- ggplot(data, aes(x=factor(Group, levels= c("0-1", "2-4", "5-6")), y= Proportion)) +
  ...
  p_dwm_diagnosis <- ggplot(data, aes(x=factor(Diagnosis, levels= c("Control", "Case")), y=countG)) +
    geom_quasirandom(size=0.1) +
    geom_boxplot(alpha=0.1, col= 'black', width=0.25) +
    facet_wrap(~cellType, nrow=2) + theme_classic() +
...
for (j in 1:length(reads_tc_list)) { 
  ...
  p_tc <- ggplot(data, aes(x=factor(Diagnosis, levels= c("Control", "Case")), y=Proportion)) +
    geom_quasirandom(size=0.1) +
    geom_boxplot(alpha=0.1, col= 'black', width=0.25) +
    facet_wrap(~cellType, nrow=2) + theme_classic() +
    ylab('Proportion of Counts') +
  ...
  p_tc_2 <- ggplot(data, aes(x=factor(Group, levels=c("0-1", "2-4", "5-6")), y=Proportion)) +
    geom_quasirandom(size=0.1) + geom_boxplot(alpha=0.1, col="black", width=0.25) +
    facet_wrap(~cellType, nrow=2) + theme_classic() +
    ylab('Proportion of Counts') + xlab("Patient Braak Stage") +
    stat_pvalue_manual(corr2, label= 'p.adj', step.group.by= 'cellType', step.increase= 0.15) +
  ...
  p_tc_diagnosis <- ggplot(data, aes(x=factor(Diagnosis, levels= c("Control", "Case")), y=countG)) +
    geom_quasirandom(size=0.1) +
    geom_boxplot(alpha=0.1, col= 'black', width=0.25) +
    facet_wrap(~cellType, nrow=2) + theme_classic() +
    ylab('Counts Per Million') +
  ...
  p_tc_stage <- ggplot(data, aes(x=factor(Group, levels=c("0-1", "2-4", "5-6")), y=countG)) +
    geom_quasirandom(size=0.1) + geom_boxplot(alpha=0.1, col="black", width=0.25) +
    facet_wrap(~cellType, nrow=2) + theme_classic() +
    ylab('Counts Per Million') + xlab("Patient Braak Stage") +
    stat_pvalue_manual(corr4, label= 'p.adj', step.group.by= 'cellType', step.increase= 0.15) +
